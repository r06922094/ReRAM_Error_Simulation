...
def crossbar(x, w, IL, FL, WL, unit, error_list, computeType, op_name='', padding_type='SAME'):
    x_bit = tf.py_func(decompose_bit, [x, IL, FL, WL, op_name+'_x'], tf.float32)
    w_bit = tf.py_func(decompose_bit, [w, IL, FL, WL, op_name+'_w'], tf.float32)
    w_bit_unit, count_cell = tf.py_func(decompose_unit, [w_bit, unit, computeType, op_name], [tf.float32, tf.float32])
    return compute_and_compose(x_bit, w_bit_unit, count_cell, IL, FL, WL, w.shape, unit, computeType, error_list, padding_type)

def decompose_bit(x, IL, FL, WL, op_name):
    print('decompose_bit: ', op_name)
...
def decompose_unit(w, unit, computeType, op_name):
    print('decompose_unit: ', op_name)
    w = np.int8(w)
...
def compute_and_compose(x, w, count, IL, FL, WL, shape, unit, computeType, error_list, padding_type):
    global ERR_LIST
    ERR_LIST = error_list
    ...
    for b_x in range(WL):
        ...
        for a in range(iterate):
            ...
            # sign bit:
            if computeType == 0: # Conv2d
                compute_result = tf.nn.conv2d(x[b_x], w[a][0], strides=[1,1,1,1], padding=padding_type)
                count_result = tf.nn.conv2d(x[b_x], count[a][0], strides=[1,1,1,1], padding=padding_type)
            else: # Matmul
            ...
            for b_w in range(1,WL):
                shift_w -= 1
                if computeType == 0: # Conv2d
                    compute_result = tf.nn.conv2d(x[b_x], w[a][b_w], strides=[1,1,1,1], padding=padding_type) 
                    count_result = tf.nn.conv2d(x[b_x], count[a][b_w], strides=[1,1,1,1], padding=padding_type)
                else: # Matmul
